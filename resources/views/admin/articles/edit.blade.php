...
<div class="px-8 py-6 max-w-7xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-black text-slate-800 tracking-tight">Edit Article</h2>
            <p class="text-sm text-slate-500 font-medium">Update content blocks, images, and settings.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('blog.show', $article->slug) }}" target="_blank" class="text-sm font-bold text-blue-600 hover:text-blue-800 bg-blue-50 px-3 py-1.5 rounded-lg transition">
                <i class="fa-solid fa-eye mr-1"></i> View Live
            </a>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <input type="text" name="title" value="{{ old('title', $article->title) }}" required class="w-full bg-slate-50 border-0 text-slate-900 text-xl font-black rounded-xl p-4 focus:ring-2 focus:ring-brand-primary placeholder:font-medium placeholder:text-slate-400">
                </div>

                <!-- Block Builder (loads existing blocks from $article->content) -->
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <div class="mb-4">
                        <img src="{{ $article->thumbnail_url }}" alt="Current" class="w-full h-32 object-cover rounded-xl border border-slate-200">
                    </div>
                    ...
                    <p class="text-[10px] text-slate-400 font-medium mt-2">Upload new to replace.</p>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                    ...
                    <div>
                        ...
                        <div x-data="{
                            open: false,
                            mode: 'select',
                            selected: '{{ old('category', $article->category ?? '') }}'.split(',').map(s => s.trim()).filter(s => s),
                            newCat: '',
                    ...
                    <div>
                        ...
                        <input type="text" name="author" value="{{ old('author', $article->author) }}" class="w-full bg-slate-50 border border-slate-200 text-slate-700 text-sm font-medium rounded-lg p-3 focus:ring-2 focus:ring-brand-primary" placeholder="Author name">
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer group mb-6 bg-slate-50 p-4 rounded-xl border border-slate-200 hover:border-brand-primary/30 transition">
                        ...
                        <div>
                            <span class="text-sm font-bold text-slate-700 block">Published Status</span>
                            <span class="text-[10px] text-slate-500 font-medium leading-none">Turn off to revert to draft.</span>
                        </div>
                    </label>
