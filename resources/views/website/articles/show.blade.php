        ...
        <article class="bg-white rounded-[4rem] shadow-2xl overflow-hidden border border-slate-100">
                ...
                @if($article->category)
                    <div class="absolute top-10 left-10 flex flex-wrap gap-3 z-20">
                        @foreach(array_map('trim', explode(',', $article->category)) as $cat)
                            <span class="px-6 py-2 bg-brand-primary text-white text-[9px] font-black uppercase tracking-widest rounded-full shadow-2xl">{{ $cat }}</span>
                        @endforeach
                    </div>
                @endif
            ...
            <div class="p-12 md:p-24 space-y-12">
                    ...
                    @if($article->author)
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <span class="text-sm font-black text-slate-900 uppercase tracking-tight">{{ $article->author }}</span>
                            </div>
                        </div>
                    @endif
                        ...
                        @foreach($blocks as $block)
                            @switch($block['type'] ?? '')
                                @case('heading')
                                    ...
                                    <{{ $tag }} class="text-4xl font-black text-slate-900 leading-none uppercase tracking-tighter pt-8">
                                ...
                                @case('image')
                                    @if($block['url'] ?? '')
                                        <figure class="rounded-[3rem] overflow-hidden shadow-2xl border border-slate-100">
                                            <img src="{{ $block['url'] }}" alt="{{ $block['caption'] ?? '' }}" class="w-full grayscale hover:grayscale-0 transition-all duration-1000">
                                            @if($block['caption'] ?? '')
                                ...
                                @case('quote')
                                    <div class="relative p-16 rounded-[3.5rem] bg-slate-950 overflow-hidden shadow-2xl">
                                        ...
                                        <div class="space-y-6 relative z-10">
                                            ...
                                            @if($block['author'] ?? '')
                                                <cite class="block text-[10px] font-black text-brand-primary uppercase tracking-[0.4em] not-italic">— {{ $block['author'] }}</cite>
                                            @endif
                                        </div>
                                    </div>
                                    @break
                ...
                <div class="pt-16 border-t border-slate-100">
                    <a href="{{ route('blog.index') }}" class="group inline-flex items-center gap-4 text-[11px] font-black text-slate-900 uppercase tracking-[0.3em] hover:text-brand-primary transition-all">
                ...
                @foreach($related as $rel)
                    <article class="group relative rounded-[3rem] bg-white border border-slate-100 overflow-hidden shadow-xl transition-all duration-700 hover:-translate-y-4 hover:shadow-brand-primary/20">
                        ...
                        <div class="p-10 space-y-6">
                            <div class="text-[9px] font-black text-brand-primary uppercase tracking-widest">{{ $rel->category ?? 'Institutional Node' }}</div>
                            ...
                            <div class="pt-6 border-t border-slate-50">
                                <a href="{{ route('blog.show', $rel->slug) }}" class="inline-flex items-center gap-4 text-[9px] font-black text-slate-900 uppercase tracking-[0.3em] hover:gap-6 transition-all">
