  ...
  <div class="container space-y-16">
    ...
    <div class="space-y-12 animate-on-scroll">
     ...
     <div class="grid grid-cols-1 sm:grid-cols-2 gap-px bg-slate-200 border border-slate-200">
      ...
      <div class="flex items-center gap-6 p-8 bg-white group hover:bg-slate-950 transition-all duration-500">
       ...
       <span class="text-[10px] font-bold uppercase text-slate-950 tracking-[0.2em] hover:text-white transition-colors">{{ $item }}</span>
      </div>
      @endforeach
     </div>
    </div>
   ...
   <div class="grid md:grid-cols-3 gap-6">
    ...
    <div class="group relative bg-white p-16 hover:bg-slate-950 transition-all duration-700">
     ...
     <h4 class="text-3xl font-bold text-slate-950 mb-8 uppercase tracking-tight hover:text-white transition-colors leading-none">{{ $step['t'] }}</h4>
     <p class="text-slate-500 text-[10px] font-bold leading-relaxed uppercase tracking-[0.3em] hover:text-slate-400 transition-colors">{{ $step['d'] }}</p>
    </div>
    @endforeach
   </div>
     ...
     <div class="grid grid-cols-2 gap-px bg-white/5 border border-white/10">
      ...
      <div class="p-12 bg-transparent hover:bg-brand-primary transition-all duration-700 group">
       <div class="w-16 h-16 bg-white/5 border border-white/10 flex items-center justify-center mb-10 hover:bg-brand-primary-dark hover:border-brand-primary-dark transition-all">
        <i class="fa-solid fa-{{ $dev['icon'] }} text-2xl text-brand-primary hover:text-white"></i>
       </div>
       <h4 class="text-[10px] font-bold uppercase text-white mb-2 tracking-[0.2em] hover:text-slate-950 transition-colors">{{ $dev['title'] }}</h4>
       <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest hover:text-slate-900 transition-colors">{{ $dev['desc'] }}</p>
      </div>
      @endforeach
     </div>
    </div>
   ...
   <div class="grid lg:grid-cols-2 gap-24 items-center">
    <div class="order-2 lg:order-1 grid gap-px bg-slate-200 border border-slate-200">
     ...
     <div class="bg-white p-12 flex gap-10 items-center group hover:bg-slate-950 transition-all duration-700">
      <span class="text-7xl font-bold text-slate-100 hover:text-brand-primary transition-colors leading-none">{{ $struct['n'] }}</span>
      <div class="space-y-4">
       <h5 class="font-bold text-slate-950 uppercase text-2xl tracking-tight hover:text-white transition-colors leading-none">{{ $struct['t'] }}</h5>
       <p class="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] hover:text-slate-400 transition-colors leading-relaxed">{{ $struct['d'] }}</p>
      </div>
     </div>
     @endforeach
    </div>
   ...
   <div class="relative bg-slate-900 p-24 lg:p-32 overflow-hidden group border-t border-slate-800 text-center">
    ...
    <div class="relative z-10 space-y-16 max-w-5xl mx-auto">
     ...
     <div class="flex flex-col sm:flex-row items-center justify-center gap-6">
      ...
      <a href="{{ route('register') }}" class="w-full sm:w-auto px-16 py-8 bg-white text-slate-950 font-bold uppercase tracking-[0.4em] text-[11px] hover:bg-brand-primary hover:text-white transition-all shadow-4xl">
       Acquire Membership
      </a>
     </div>
    </div>
