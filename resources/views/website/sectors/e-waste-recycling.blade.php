...
@section('content')
    ...
    <section class="relative pt-64 pb-32 bg-slate-950 overflow-hidden border-b border-slate-900">
        ...
        <div class="absolute inset-0 opacity-20">
            <img src="{{ asset('images/sectors/e-waste-recycling.jpeg') }}" class="w-full h-full object-cover grayscale">
        </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-px bg-slate-200 border border-slate-200 shadow-3xl">
                    ...
                    @foreach($process as $p)
                        <div class="p-16 bg-white text-center group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
                            ...
                            <p class="text-[10px] font-black text-slate-900 group-hover:text-white uppercase tracking-[0.3em] leading-tight transition-colors">{{ $p['label'] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="grid grid-cols-2 gap-px bg-white/10 border border-white/10 shadow-2xl">
                                @foreach(['Rapid Consumption', 'Shorter Lifecycles', 'IT Upgrades', 'ESG Mandates'] as $driver)
                                    <span class="px-8 py-6 bg-slate-950 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] text-center hover:bg-yellow-600 hover:text-slate-950 transition-colors cursor-default">{{ $driver }}</span>
                                @endforeach
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 pt-8">
                         @foreach(['Delhi NCR', 'Mumbai', 'Bengaluru', 'Chennai', 'Hyderabad', 'Pune'] as $hub)
                            <div class="flex items-center gap-4 group/item">
                                ...
                                <span class="text-[12px] font-black uppercase text-slate-600 group-hover:text-slate-400 tracking-[0.2em] transition-colors leading-relaxed group-hover/item:text-yellow-600">{{ $hub }}</span>
                            </div>
                         @endforeach
                    </div>
                </div>
                    ...
                    <ul class="space-y-6">
                        @foreach([
                            ...
                            <li class="flex items-start gap-8 group/item">
                                ...
                                <span class="text-[14px] font-black uppercase text-slate-600 tracking-[0.2em] transition-colors leading-relaxed group-hover/item:text-slate-950 italic">{{ $driver }}</span>
                            </li>
                        @endforeach
                    </ul>
                </div>
                    ...
                    @foreach(['Precious Metal Recovery', 'PCB Recycling', 'Battery Recycling', 'Refurbishment', 'Data Destruction', 'Reverse Logistics'] as $seg)
                        <div class="p-16 bg-white text-center group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
                            <div class="absolute top-0 left-0 w-full h-1 bg-yellow-600 scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-500"></div>
                             <p class="text-[12px] font-black uppercase text-slate-600 group-hover:text-white tracking-[0.2em] transition-colors italic leading-tight">{{ $seg }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
