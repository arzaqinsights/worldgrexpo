...
@section('content')
 ...
 <section class="relative pt-40 pb-20 bg-slate-950 overflow-hidden border-b border-slate-900">
  ...
  <div class="absolute inset-0 opacity-20">
   <img src="{{ asset('images/sectors/plastic-machinery.jpg') }}" class="w-full h-full object-cover">
  </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
     ...
     @foreach($machinery as $m)
      <div class="p-12 bg-slate-950 group/item hover:bg-indigo-600 transition-all duration-500">
       ...
       <h4 class="text-xl font-bold uppercase text-white group-hover/item:text-slate-950 mb-4 transition-colors ">{{ $m['title'] }}</h4>
       <p class="text-[10px] font-bold uppercase text-slate-500 group-hover/item:text-slate-900 tracking-widest leading-relaxed transition-colors ">{{ $m['desc'] }}</p>
      </div>
     @endforeach
    </div>
   </div>
