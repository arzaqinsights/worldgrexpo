...
@section('content')
    ...
    <section class="relative pt-64 pb-32 bg-slate-950 overflow-hidden border-b border-slate-900">
        ...
        <div class="absolute inset-0 opacity-20">
            <img src="{{ asset('images/sectors/start-up-ecosystem.jpg') }}" class="w-full h-full object-cover grayscale">
        </div>
                            ...
                            <div class="grid grid-cols-3 gap-px bg-white/10 border border-white/10 shadow-2xl">
                                @foreach(['USA', 'UK', 'Germany', 'Singapore', 'Israel', 'UAE'] as $hub)
                                    <div class="p-6 bg-slate-950 flex flex-col items-center justify-center gap-4 group/item hover:bg-fuchsia-600 transition-all duration-500">
                                        <span class="text-[9px] font-black uppercase text-slate-300 group-hover:text-slate-950 tracking-[0.1em] text-center">{{ $hub }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
