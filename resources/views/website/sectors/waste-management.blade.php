...
@section('content')
    ...
    <section class="relative pt-64 pb-32 bg-slate-950 overflow-hidden border-b border-slate-900">
        ...
        <div class="absolute inset-0 opacity-20">
            <img src="{{ asset('images/sectors/waste-management.jpg') }}" class="w-full h-full object-cover grayscale">
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-px bg-slate-200 border border-slate-200 shadow-3xl">
                    ...
                    @foreach($segments as $s)
                        <div class="p-16 bg-white {{ $s['col'] ?? '' }} flex flex-col items-center justify-center text-center group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
                            ...
                            <h4 class="text-[12px] font-black uppercase text-slate-950 group-hover:text-white transition-colors tracking-[0.15em] mb-2 italic">{{ $s['title'] }}</h4>
                            <p class="text-[10px] font-black uppercase text-slate-400 tracking-[0.1em] leading-tight">{{ $s['desc'] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
