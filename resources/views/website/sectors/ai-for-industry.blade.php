...
@section('content')
    ...
    <section class="relative pt-64 pb-32 bg-slate-950 overflow-hidden border-b border-slate-900">
        ...
        <div class="absolute inset-0 opacity-20">
            <img src="{{ asset('images/sectors/ai-for-industry.jpg') }}" class="w-full h-full object-cover grayscale">
        </div>
        ...
        <div class="container relative z-10 text-center space-y-12">
            <div class="inline-flex items-center gap-4 px-6 py-2 bg-white/5 border border-white/10">
                ...
                <span class="text-white text-[10px] font-black tracking-[0.4em] uppercase">Intelligent Systems Era</span>
            </div>
        ...
        <div class="container space-y-48">
            ...
            <div class="grid lg:grid-cols-2 gap-px bg-slate-200 border border-slate-200">
                <div class="p-16 lg:p-24 bg-white space-y-12">
                    <div class="section-heading text-left">
                        ...
                        <h2 class="text-5xl font-black uppercase tracking-tighter">The Horizontal <br><span class="text-brand-primary">Capability Layer.</span></h2>
                        <div class="accent-line"></div>
                    </div>
                    
                    <div class="space-y-8 text-2xl text-slate-500 font-medium leading-relaxed italic border-l-4 border-slate-100 pl-10">
                        <p>
                            Artificial Intelligence is no longer sector-specific—it is a <strong class="text-slate-900 font-black uppercase tracking-tight">horizontal capability layer</strong> transforming every major industry.
                        </p>
                        <p>
                ...
                <div class="grid grid-cols-2 gap-px bg-slate-200">
                    @foreach(['Manufacturing', 'Sustainability', 'Agriculture', 'Healthcare'] as $idx => $svc)
                        <div class="p-12 bg-slate-50 flex flex-col items-center justify-center text-center group hover:bg-brand-primary transition-all duration-700">
                            <div class="text-6xl font-black text-brand-primary group-hover:text-white transition-colors mb-6">
                                0{{ $idx + 1 }}
                            </div>
                            <p class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 group-hover:text-slate-900 transition-colors">{{ $svc }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="space-y-24">
                <div class="section-heading text-center">
                    ...
                    <h2 class="text-6xl font-black uppercase tracking-tighter">Primary Focus <span class="text-brand-primary">Architectures.</span></h2>
                ...
                <div class="grid lg:grid-cols-3 gap-px bg-slate-200 border border-slate-200 shadow-3xl">
                    ...
                    @foreach($focusAreas as $area)
                        <div class="p-16 bg-white space-y-12 group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
                            ...
                            <div class="space-y-4">
                                <div class="text-brand-primary text-[10px] font-black uppercase tracking-[0.4em]">{{ $area['tag'] }}</div>
                                <h3 class="text-3xl font-black text-slate-900 group-hover:text-white uppercase tracking-tighter">{{ $area['title'] }}</h3>
                            </div>
                            <ul class="space-y-6">
                                @foreach($area['items'] as $item)
                                    <li class="flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-slate-300 transition-colors">
                            ...
                            <div class="pt-8 border-t border-slate-100 group-hover:border-white/10">
                                <div class="text-[10px] font-black text-brand-primary uppercase tracking-[0.3em]">{{ $area['impact'] }}</div>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <!-- Investor Snapshot -->
            <div class="p-24 lg:p-32 bg-slate-950 relative overflow-hidden group shadow-3xl">
                <div class="absolute inset-0 z-0">
                    <div class="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:40px_40px]"></div>
                </div>
                <div class="relative z-10 grid lg:grid-cols-2 gap-px bg-white/10 border border-white/10 shadow-3xl">
                    <div class="p-16 lg:p-24 bg-slate-950 space-y-16">
                        <div class="space-y-8">
                            ...
                            <h2 class="text-6xl md:text-8xl font-black text-white leading-[0.85] tracking-tighter uppercase">
                                Investor <br>
                        ...
                        <div class="grid gap-6">
                            @foreach([
                                ...
                                <div class="flex items-center gap-6 group">
                                    ...
                                    <span class="text-xs font-black uppercase text-slate-400 group-hover:text-white transition-colors tracking-[0.2em]">{{ $hl }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-px bg-white/10">
                        @foreach(['Generative AI', 'AI SaaS', 'AI Infrastructure', 'Industry Solutions'] as $tag)
                            <div class="p-12 bg-slate-950 flex flex-col items-center justify-center text-center group hover:bg-brand-primary transition-all duration-700">
                                <span class="text-[10px] font-black uppercase tracking-[0.4em] text-white group-hover:text-slate-950 transition-colors">{{ $tag }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="grid lg:grid-cols-2 gap-px bg-slate-200 border border-slate-200">
                <div class="p-16 lg:p-24 bg-white space-y-16">
                    <div class="section-heading text-left">
                        ...
                        <h2 class="text-5xl font-black uppercase tracking-tighter">Global AI <br><span class="text-brand-primary">Summit.</span></h2>
                        <div class="accent-line"></div>
                    </div>
                    <div class="p-16 bg-slate-950 text-white space-y-8 relative group">
                        ...
                        <p class="text-[10px] font-black text-brand-primary uppercase tracking-[0.3em]">Summit Theme</p>
                        <p class="text-4xl font-black text-white leading-tight tracking-tight uppercase italic">
                            "AI for Viksit Bharat 2047: Driving Intelligent, Sustainable & Inclusive Growth"
                        </p>
                    </div>
                    <div class="flex flex-wrap gap-px bg-slate-200 border border-slate-200">
                        @foreach(['Global Leaders', 'Policy Makers', 'Startup Founders', 'Academic Experts'] as $spk)
                             <div class="px-8 py-6 bg-slate-950 text-white text-[10px] font-black uppercase tracking-[0.3em] flex-1 text-center whitespace-nowrap">{{ $spk }}</div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="p-16 lg:p-24 bg-slate-950 text-white space-y-16 relative overflow-hidden group shadow-3xl">
                    ...
                    <div class="relative z-10 space-y-12">
                        <h4 class="text-4xl font-black uppercase tracking-tighter text-brand-accent">Conference Tracks</h4>
                        <div class="grid gap-px bg-white/10 border border-white/10">
                            ...
                            <div class="flex items-center gap-8 p-8 bg-slate-950 group/track hover:bg-brand-primary transition-all duration-500">
                                ...
                                <p class="text-xs font-black uppercase tracking-[0.2em] text-slate-400 group-hover/track:text-slate-950 transition-colors">{{ $track }}</p>
                            </div>
                            @endforeach
                        </div>
                    </div>
