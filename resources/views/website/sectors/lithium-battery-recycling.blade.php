...
@section('content')
    ...
    <section class="relative pt-64 pb-32 bg-slate-950 overflow-hidden border-b border-slate-900">
        ...
        <div class="absolute inset-0 opacity-20">
            <img src="{{ asset('images/sectors/lithium-battery-recycling.webp') }}" class="w-full h-full object-cover grayscale">
        </div>
                ...
                <div class="grid grid-cols-2 gap-px bg-slate-200 shadow-3xl">
                    ...
                    @foreach($sources as $s)
                        <div class="p-10 bg-white flex flex-col items-center justify-center text-center group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
                            ...
                            <i class="fa-solid fa-{{ $s['icon'] }} text-4xl mb-6 text-slate-200 group-hover:text-teal-600 transition-all duration-500"></i>
                            <p class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 group-hover:text-white transition-colors">{{ $s['label'] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
                    ...
                    <div class="relative z-10 space-y-16">
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="grid grid-cols-2 gap-px bg-white/10 border border-white/10 shadow-2xl">
                                @foreach(['EV Sales Growth', 'Mineral Shortages', 'ESG Pressure', 'Mandatory Mandates'] as $driver)
                                    <div class="p-8 bg-slate-950 text-center text-[10px] font-black uppercase tracking-[0.1em] text-slate-400 hover:bg-teal-600 hover:text-slate-950 transition-all cursor-default">{{ $driver }}</div>
                                @endforeach
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-px bg-white/5 border border-white/5">
                        @foreach([
                            ...
                            <div class="p-10 bg-slate-950 group/item hover:bg-teal-600 transition-all duration-500 relative overflow-hidden">
                                ...
                                <i class="fa-solid fa-{{ $tech['icon'] }} text-3xl mb-6 text-teal-600 group-hover/item:text-slate-950 transition-colors"></i>
                                <p class="text-[12px] font-black uppercase text-slate-400 group-hover/item:text-slate-950 tracking-[0.2em] leading-tight transition-colors italic">{{ $tech['label'] }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-px bg-slate-100 border border-slate-100 shadow-2xl">
                         @foreach([
                            ...
                            <div class="p-10 bg-white group/val hover:bg-slate-950 transition-all duration-500 relative overflow-hidden">
                                ...
                                <i class="fa-solid fa-{{ $val['icon'] }} text-3xl mb-6 text-teal-600 group-hover/val:text-white transition-colors"></i>
                                <p class="text-[11px] font-black uppercase text-slate-500 group-hover/val:text-white tracking-[0.2em] leading-tight transition-colors italic">{{ $val['label'] }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="p-16 lg:p-24 bg-slate-950 text-white space-y-16 relative overflow-hidden group shadow-3xl border border-white/5">
                    ...
                    <div class="relative z-10 space-y-12">
                        ...
                        <div class="grid grid-cols-2 gap-px bg-white/5 pt-12 border border-white/10">
                             @foreach(['50-100 RECYCLERS', 'BLACK MASS HUB', 'URBAN MINING', 'CLOSED LOOP'] as $tag)
                                <span class="p-8 bg-slate-950 border border-white/5 text-[10px] font-black uppercase tracking-[0.4em] text-teal-300 text-center hover:bg-teal-600 hover:text-slate-950 transition-all cursor-default">{{ $tag }}</span>
                             @endforeach
                        </div>
                    </div>
                </div>
