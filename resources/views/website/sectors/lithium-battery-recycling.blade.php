...
@section('content')
 ...
 <section class="relative pt-40 pb-20 bg-slate-950 overflow-hidden border-b border-slate-900">
  ...
  <div class="absolute inset-0 opacity-20">
   <img src="{{ asset('images/sectors/lithium-battery-recycling.webp') }}" class="w-full h-full object-cover">
  </div>
    ...
    <div class="grid grid-cols-2 gap-px bg-slate-200 shadow-sm">
     ...
     @foreach($sources as $s)
      <div class="p-10 bg-slate-50 rounded-2xl p-8 flex flex-col items-center text-center group hover:shadow-lg transition-all">
       ...
       <p class="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 group-hover:text-white transition-colors">{{ $s['label'] }}</p>
      </div>
     @endforeach
    </div>
   </div>
      ...
      <div class="space-y-8">
       ...
       <div class="grid grid-cols-2 gap-4">
        @foreach(['EV Sales Growth', 'Mineral Shortages', 'ESG Pressure', 'Mandatory Mandates'] as $driver)
         <div class="p-8 bg-slate-950 text-center text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400 hover:bg-teal-600 hover:text-slate-950 transition-all cursor-default">{{ $driver }}</div>
        @endforeach
       </div>
      </div>
     </div>
     ...
     <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      @foreach([
       ...
       <div class="p-10 bg-slate-950 group/item hover:bg-teal-600 transition-all duration-500 relative overflow-hidden">
        ...
        <p class="text-[12px] font-bold uppercase text-slate-400 group-hover/item:text-slate-950 tracking-[0.2em] leading-tight transition-colors ">{{ $tech['label'] }}</p>
       </div>
      @endforeach
     </div>
    </div>
    ...
    <div class="p-12 bg-white space-y-16 border-l border-slate-200">
     ...
     <div class="grid grid-cols-1 sm:grid-cols-2 gap-px bg-slate-100 border border-slate-100 shadow-2xl">
       ...
       <div class="p-10 bg-white group/val hover:bg-slate-950 transition-all duration-500 relative overflow-hidden">
        ...
        <p class="text-[11px] font-bold uppercase text-slate-500 group-hover/val:text-white tracking-[0.2em] leading-tight transition-colors ">{{ $val['label'] }}</p>
       </div>
      @endforeach
     </div>
    </div>
    ...
    <div class="p-8 lg:p-12 bg-slate-950 text-white space-y-16 relative overflow-hidden group shadow-sm border border-white/5">
     ...
     <div class="relative z-10 space-y-12">
      ...
      <div class="grid grid-cols-2 gap-px bg-white/5 pt-12 border border-white/10">
        ...
        <span class="p-8 bg-slate-950 border border-white/5 text-[10px] font-bold uppercase tracking-[0.4em] text-teal-300 text-center hover:bg-teal-600 hover:text-slate-950 transition-all cursor-default">{{ $tag }}</span>
        @endforeach
      </div>
     </div>
    </div>
