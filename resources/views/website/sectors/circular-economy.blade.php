      ...
      <div class="space-y-8">
       ...
       <div class="grid grid-cols-2 gap-px bg-white/10 border border-white/10">
        @foreach(['Raw Material Costs', 'Resource Scarcity', 'Climate Mitigation', 'ESG Mandates'] as $driver)
         <span class="px-8 py-6 bg-slate-950 text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] text-center hover:bg-teal-600 hover:text-slate-950 transition-all cursor-default">{{ $driver }}</span>
        @endforeach
       </div>
      </div>
     </div>
       ...
       <div class="grid grid-cols-2 md:grid-cols-3 gap-6 shadow-sm">
        @foreach(['Waste Management', 'Sustainable Packaging', 'Renewable Energy', 'Water Reuse', 'Industrial Efficiency', 'Green Manufacturing'] as $area)
         <div class="p-8 bg-white flex items-center gap-6 group/item hover:bg-slate-950 transition-all duration-500">
          ...
          <span class="text-[10px] font-bold uppercase text-slate-600 group-hover/item:text-white tracking-[0.1em] leading-tight transition-colors">{{ $area }}</span>
         </div>
        @endforeach
       </div>
      </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6 shadow-sm">
     ...
     @foreach($pillars as $p)
      <div class="p-8 bg-slate-50 rounded-2xl text-center group hover:shadow-lg transition-all relative overflow-hidden">
       ...
       <h4 class="text-2xl font-bold text-slate-950 group-hover:text-white mb-4 leading-tight transition-colors">{{ $p['title'] }}</h4>
       <p class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] leading-tight transition-colors">{{ $p['desc'] }}</p>
      </div>
     @endforeach
    </div>
   </div>
