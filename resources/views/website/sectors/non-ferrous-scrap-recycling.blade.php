    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 shadow-sm">
     ...
     @foreach($metals as $metal)
      <div class="p-12 bg-slate-50 rounded-2xl p-8 flex flex-col items-center text-center group hover:shadow-lg transition-all">
       ...
       <h4 class="text-[12px] font-bold uppercase text-slate-950 group-hover:text-white transition-colors tracking-[0.15em] ">{{ $metal['title'] }}</h4>
      </div>
     @endforeach
    </div>
   </div>
       ...
       <div class="grid grid-cols-2 gap-4">
        @foreach(['EV Mfg', 'Renewables', 'Infra Expansion', 'Circular Economy'] as $driver)
         <div class="flex items-center gap-3">
          ...
          <span class="text-[10px] font-bold uppercase text-slate-400 tracking-widest">{{ $driver }}</span>
         </div>
        @endforeach
       </div>
      </div>
      ...
      <div class="grid grid-cols-1 gap-6">
       @foreach([
        ...
        <div class="flex items-center gap-6 group/item">
         ...
         <span class="text-lg font-bold uppercase text-slate-400 group-hover:text-white transition-colors tracking-widest ">{{ $reason }}</span>
        </div>
       @endforeach
      </div>
     </div>
