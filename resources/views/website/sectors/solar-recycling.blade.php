...
@section('content')
    ...
    <section class="relative pt-64 pb-32 bg-slate-950 overflow-hidden border-b border-slate-900">
        ...
        <div class="absolute inset-0 opacity-20">
            <img src="{{ asset('images/sectors/solar-recycling.jpeg') }}" class="w-full h-full object-cover grayscale">
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-px bg-slate-200 border border-slate-200 shadow-3xl">
                    ...
                    @foreach($processes as $p)
                        <div class="p-16 bg-white flex flex-col items-center justify-center text-center group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
                            ...
                            <h4 class="text-[12px] font-black uppercase text-slate-950 group-hover:text-white transition-colors tracking-[0.15em] mb-2">{{ $p['title'] }}</h4>
                            <p class="text-[10px] font-black uppercase text-slate-400 tracking-[0.1em] italic leading-tight">{{ $p['desc'] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="grid grid-cols-2 gap-px bg-white/10 border border-white/10 shadow-2xl">
                                @foreach(['Rapid Installed Growth', 'WEEE Regulations', 'High Mineral Value', 'ESG Mandates'] as $driver)
                                    <div class="p-8 bg-slate-950 text-center text-[10px] font-black uppercase tracking-[0.1em] text-slate-400 hover:bg-amber-600 hover:text-slate-950 transition-all cursor-default">{{ $driver }}</div>
                                @endforeach
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-px bg-slate-200 border border-slate-200 shadow-3xl">
                    <div class="p-12 bg-slate-950 text-white space-y-8 flex flex-col justify-center border-r border-white/5">
                        ...
                        @foreach(['Market Size 2025', 'Forecast Value', 'CAGR', 'Market Stage'] as $param)
                            <p class="text-[10px] font-black uppercase text-slate-500 tracking-[0.4em]">{{ $param }}</p>
                        @endforeach
                    </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-px bg-slate-200 border border-slate-200 shadow-3xl">
                    ...
                    @foreach($materials as $m)
                        <div class="p-10 bg-white flex flex-col items-center justify-center text-center group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
                            ...
                            <h4 class="text-[12px] font-black uppercase text-slate-950 group-hover:text-white transition-colors tracking-[0.15em] mb-2 italic">{{ $m['title'] }}</h4>
                            <p class="text-[9px] font-black uppercase text-slate-400 tracking-[0.1em] leading-tight">{{ $m['desc'] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
                    ...
                    <div class="relative z-10 space-y-12">
                        ...
                        <div class="grid grid-cols-2 gap-px bg-white/5 pt-12 border border-white/10">
                             @foreach(['300+ OPERATORS', 'E-WASTE SYNERGY', 'GREEN MINING', 'FORMALIZING'] as $tag)
                                <span class="p-8 bg-slate-950 border border-white/5 text-[10px] font-black uppercase tracking-[0.4em] text-amber-300 text-center hover:bg-amber-600 hover:text-slate-950 transition-all cursor-default">{{ $tag }}</span>
                             @endforeach
                        </div>
                    </div>
                </div>
