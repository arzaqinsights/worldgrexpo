            ...
            <div class="flex flex-col lg:flex-row gap-16 items-center">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                        <div class="flex items-center gap-6 p-6 bg-white/5 border border-white/10 rounded-sm group transition-all">
                            ...
                            <div>
                                ...
                                <span class="block text-xl font-black text-white italic uppercase tracking-tighter">{{ $event->event_date->format('d M, Y') }}</span>
                                <span class="block text-xs font-bold text-brand-accent uppercase tracking-widest mt-1">{{ $event->event_date->format('h:i A') }} IST</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-6 p-6 bg-white/5 border border-white/10 rounded-sm group transition-all">
                            ...
                            <div>
                                ...
                                <span class="block text-xl font-black text-white italic uppercase tracking-tighter">{{ $event->location ?: 'Global Node' }}</span>
                ...
                <div class="w-full lg:w-2/5 animate-on-scroll">
                    <div class="relative rounded-sm overflow-hidden border border-white/10 shadow-4xl group">
                        <img src="{{ $event->image ? asset($event->image) : asset('images/event-placeholder.jpg') }}" alt="{{ $event->title }}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-[2000ms]">
                ...
                <aside class="w-full hidden lg:block lg:w-1/4 sticky top-40 z-30">
                    <div class="bg-slate-950 rounded-sm border border-slate-900 overflow-hidden shadow-4xl">
                                ...
                                @foreach($navItems as $item)
                                    ...
                                    @if($showItem)
                                        <a href="#{{ $item['id'] }}" 
                                           ...
                                           :class="activeSection === '{{ $item['id'] }}' ? 'bg-brand-accent text-slate-950' : 'text-slate-500 hover:bg-white/5 hover:text-white'"
                                           class="flex items-center justify-between p-5 rounded-sm text-[10px] font-black uppercase tracking-[0.3em] transition-all italic group/nav">
                                            <span class="flex items-center gap-4">
                                                <i class="fa-solid {{ $item['icon'] }} w-4 text-xs transition-transform group-hover/nav:scale-125"></i>
                                                {{ $item['label'] }}
                                            </span>
                        ...
                        <div class="p-8 bg-slate-900 border-t border-slate-800">
                            <a href="{{ $event->builder_content['about']['registration_url'] ?? url('join') }}" target="_blank" class="block w-full bg-brand-accent text-slate-950 hover:bg-white text-center py-6 rounded-sm font-black text-[10px] uppercase tracking-[0.3em] transition-all italic shadow-2xl">
                    ...
                    <div class="mt-4 bg-slate-50 rounded-sm p-8 border border-slate-100 text-left">
                        ...
                        <div class="space-y-2">
                            <a href="https://wa.me/919810690843?text=Hi,%20I'm%20interested%20in%20the%20{{ urlencode($event->title) }}%20nomination." target="_blank" class="flex items-center justify-center gap-4 w-full bg-[#25D366] text-white hover:bg-slate-950 py-5 rounded-sm font-black text-[10px] uppercase tracking-[0.3em] transition-all italic shadow-sm">
                    ...
                    <div id="about" class="scroll-mt-40 bg-slate-50 border border-slate-100 rounded-sm p-12 md:p-20 shadow-sm animate-on-scroll">
                        <div class="flex items-center gap-8 mb-16">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] italic">{{ $event->builder_content['about']['subtitle'] ?? 'Transforming the Future' }}</p>
                            </div>
                        </div>
                                        ...
                                        <div class="group bg-slate-50 border border-slate-100 rounded-sm overflow-hidden hover:bg-slate-950 transition-all duration-700 p-12 relative flex flex-col md:flex-row gap-16 items-center md:items-start">
                                            ...
                                            <div class="w-full md:w-64 h-auto overflow-hidden shrink-0 rounded-sm bg-slate-900 border border-slate-200 group-hover:border-slate-800 transition-colors shadow-2xl">
                                                @if(!empty($guest['photo']))
                                                    <img src="{{ asset($guest['photo']) }}" alt="{{ $guest['name'] }}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-[1500ms]">
                                                @else
                                            ...
                                            <div class="flex-1 text-center md:text-left">
                                                <div class="inline-flex items-center gap-4 px-4 py-2 bg-slate-950 border border-white/5 rounded-sm text-[8px] font-black text-brand-accent uppercase tracking-[0.4em] italic mb-8 group-hover:border-white/10">
                                                    <i class="fa-solid fa-star"></i> {{ $guest['title'] ?? 'Strategic Node Guest' }}
                                                </div>
                                                <h3 class="text-4xl md:text-5xl font-black text-slate-950 group-hover:text-white transition-colors mb-4 uppercase italic tracking-tighter leading-none">{{ $guest['name'] }}</h3>
                                                <p class="text-brand-primary font-black text-[10px] uppercase tracking-[0.3em] italic mb-10 group-hover:text-brand-accent transition-colors">{{ $guest['designation'] }}</p>
                                    ...
                                    @foreach($portraitGuests as $guest)
                                        <div class="bg-slate-50 border border-slate-100 rounded-sm overflow-hidden p-8 group hover:bg-slate-950 transition-all duration-700 text-left relative">
                                            ...
                                            <div class="w-full aspect-[4/5] rounded-sm overflow-hidden mb-8 border border-slate-200 group-hover:border-slate-800 transition-all shadow-xl">
                                                @if(!empty($guest['photo']))
                                                    <img src="{{ asset($guest['photo']) }}" alt="{{ $guest['name'] }}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000">
                                                @else
                                            ...
                                            <h3 class="text-xl font-black text-slate-950 group-hover:text-white transition-colors mb-2 uppercase italic tracking-tighter leading-none">{{ $guest['name'] }}</h3>
                                            <p class="text-[8px] font-black text-slate-500 group-hover:text-brand-accent transition-colors uppercase tracking-widest italic">{{ $guest['designation'] }}</p>
                                        </div>
                                    @endforeach
                                </div>
                            @endif
                                            ...
                                            <div class="bg-white border border-slate-200 rounded-sm p-8 group/pdf hover:bg-white/5 hover:border-white/10 transition-all">
                                                <div class="aspect-video bg-slate-950 rounded-sm mb-8 overflow-hidden relative border border-slate-900 group-hover/pdf:border-slate-700 shadow-2xl">
                                                    @if(isset($hl['pdf1_thumb']) && $hl['pdf1_thumb'])
                                                        <img src="{{ asset($hl['pdf1_thumb']) }}" class="w-full h-full object-cover grayscale group-hover/pdf:grayscale-0 transition-all duration-700">
                                                    @else
                                                    ...
                                                    <div class="absolute inset-0 bg-slate-950/60 opacity-0 group-hover/pdf:opacity-100 transition-all duration-700 flex items-center justify-center backdrop-blur-sm">
                                                        <a href="{{ asset($hl['pdf1_path']) }}" target="_blank" class="bg-brand-accent text-slate-950 px-8 py-4 rounded-sm text-[10px] font-black uppercase tracking-[0.3em] italic shadow-4xl hover:bg-white transition-colors">
                                                            Acquire Document <i class="fa-solid fa-up-right-from-square ml-4"></i>
                                                        </a>
                                                    </div>
                                                </div>
                                                <h5 class="text-[10px] font-black text-slate-950 uppercase tracking-[0.3em] group-hover/pdf:text-white transition-colors italic leading-tight">{{ $hl['pdf1_name'] ?: 'Strategic Intelligence' }}</h5>
                                            </div>
                                        @endif
                                            ...
                                            <div class="bg-white border border-slate-200 rounded-sm p-8 group/pdf hover:bg-white/5 hover:border-white/10 transition-all">
                                                <div class="aspect-video bg-slate-950 rounded-sm mb-8 overflow-hidden relative border border-slate-900 group-hover/pdf:border-slate-700 shadow-2xl">
                                                    @if(isset($hl['pdf2_thumb']) && $hl['pdf2_thumb'])
                                                        <img src="{{ asset($hl['pdf2_thumb']) }}" class="w-full h-full object-cover grayscale group-hover/pdf:grayscale-0 transition-all duration-700">
                                                    @else
                                                        <div class="w-full h-full flex flex-col items-center justify-center gap-4">
                                                            <i class="fa-solid fa-file-pdf text-5xl text-brand-accent/20 group-hover/pdf:scale-125 transition-transform"></i>
                                                    ...
                                                    @endif
                                                    <div class="absolute inset-0 bg-slate-950/60 opacity-0 group-hover/pdf:opacity-100 transition-all duration-700 flex items-center justify-center backdrop-blur-sm">
                                                        <a href="{{ asset($hl['pdf2_path']) }}" target="_blank" class="bg-brand-accent text-slate-950 px-8 py-4 rounded-sm text-[10px] font-black uppercase tracking-[0.3em] italic shadow-4xl hover:bg-white transition-colors">
                                                            Acquire Document <i class="fa-solid fa-up-right-from-square ml-4"></i>
                                                        </a>
                                                    </div>
                                                </div>
                                                <h5 class="text-[10px] font-black text-slate-950 uppercase tracking-[0.3em] group-hover/pdf:text-white transition-colors italic leading-tight">{{ $hl['pdf2_name'] ?: 'Resource Intelligence' }}</h5>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                    ...
                    @if($excellenceAwards->count() > 0)
                        <div id="excellence" class="scroll-mt-40 bg-white border border-slate-100 rounded-sm p-12 md:p-20 shadow-sm animate-on-scroll">
                            <div class="flex items-center justify-between mb-16">
                                ...
                                <a href="{{ route('excellence.index') }}" class="hidden md:inline-flex items-center gap-4 text-slate-950 text-[10px] font-black uppercase tracking-[0.4em] italic hover:text-brand-accent transition-all group/all">
                                ...
                                @foreach($excellenceAwards as $award)
                                    <div class="group bg-slate-950 border border-slate-900 rounded-sm overflow-hidden hover:bg-slate-900 transition-all duration-700 shadow-4xl">
                                        <div class="relative aspect-[4/5] border-b border-white/5">
                                            <img src="{{ asset($award->award_image) }}" alt="{{ $award->title }}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000">
                                            <div class="absolute inset-0 bg-slate-950/40 group-hover:bg-slate-950/10 transition-all duration-700"></div>
                                            <div class="absolute bottom-10 left-10 right-10">
                                                <p class="text-white font-black text-xl uppercase italic tracking-tighter leading-none group-hover:text-brand-accent transition-colors">{{ $award->title }}</p>
                                            </div>
                                        </div>
                                        <div class="p-8 flex items-center gap-6">
                                            @if($award->giver_image)
                                                <img src="{{ asset($award->giver_image) }}" class="w-12 h-12 rounded-sm grayscale group-hover:grayscale-0 border border-white/10 object-cover shadow-2xl">
                                            @endif
                                            <div>
                                                <p class="text-[10px] font-black text-white uppercase italic leading-none mb-2">{{ $award->giver_name }}</p>
                                                <p class="text-[8px] font-black text-slate-500 uppercase tracking-[0.2em] italic">{{ Str::limit($award->giver_designation, 30) }}</p>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    @if(isset($event->builder_content['gallery']) && count($event->builder_content['gallery']) > 0)
                        <div id="gallery" class="scroll-mt-40 bg-slate-50 border border-slate-100 rounded-sm p-12 md:p-20 shadow-sm animate-on-scroll">
                            <div class="mb-16 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                                ...
                                <a href="{{ route('gallery') }}" class="inline-flex items-center justify-center gap-6 bg-slate-950 hover:bg-brand-accent text-white hover:text-slate-950 px-10 py-6 rounded-sm text-[10px] font-black uppercase tracking-[0.3em] italic transition-all duration-700 shadow-4xl group/btn">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                                @foreach($event->builder_content['gallery'] as $media)
                                    <div class="relative aspect-video rounded-sm overflow-hidden bg-slate-950 border border-white/5 group shadow-4xl">
                                        ...
                                        @else
                                            <img src="{{ asset($media['url']) }}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000 group-hover:scale-105">
                                        @endif
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach($event->builder_content['testimonials'] as $testimonial)
                                    <div class="bg-slate-50 p-12 rounded-sm border border-slate-100 relative hover:bg-slate-950 transition-all duration-700 group/test shadow-sm">
                                        ...
                                        <p class="text-xl font-light italic text-slate-600 mb-12 relative z-10 group-hover/test:text-slate-400 transition-colors leading-relaxed">"{{ $testimonial['text'] }}"</p>
                                        <div class="flex items-center gap-6">
                                            <div class="w-16 h-16 rounded-sm bg-slate-200 border border-slate-300 group-hover/test:border-white/10 overflow-hidden shrink-0 shadow-xl transition-all">
                                                @if(isset($testimonial['image']) && $testimonial['image'])
                                                    <img src="{{ asset($testimonial['image']) }}" class="w-full h-full object-cover grayscale group-hover/test:grayscale-0 transition-all">
                                                @else
                                            ...
                                            <div>
                                                <p class="text-[10px] font-black text-slate-950 group-hover/test:text-white transition-colors uppercase italic tracking-[0.3em] mb-2 leading-none">{{ $testimonial['name'] }}</p>
                                                <p class="text-[8px] font-black text-slate-500 group-hover/test:text-brand-accent transition-colors uppercase tracking-widest italic">{{ $testimonial['designation'] ?? 'Institutional Representative' }}</p>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    @if(isset($event->builder_content['pricing']) && count($event->builder_content['pricing']) > 0)
                        <div id="pricing" class="scroll-mt-40 animate-on-scroll bg-slate-950 rounded-sm p-12 md:p-24 text-white relative overflow-hidden border border-slate-900 shadow-4xl">
                            ...
                            <div class="text-center mb-20 relative z-10">
                                ...
                                <h2 class="text-5xl md:text-7xl font-black text-white mb-6 uppercase italic tracking-tighter leading-none">{!! $event->builder_content['pricing_header']['heading'] ?? 'Acquire Your <span class="text-brand-accent not-italic">Summit Node</span>' !!}</h2>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
                                @foreach($event->builder_content['pricing'] as $tier)
                                    <div class="bg-white/5 backdrop-blur-md border border-white/10 rounded-sm p-12 hover:bg-white/10 hover:border-brand-accent transition-all duration-700 group relative overflow-hidden shadow-4xl">
                                        ...
                                        <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mb-4 italic group-hover:text-brand-accent transition-colors">{{ $tier['type'] }} Node</h4>
                                        <div class="flex items-end gap-4 mb-8">
                                            <span class="text-xs font-black text-brand-accent uppercase tracking-widest italic mb-2">{{ $tier['currency'] ?? 'INR' }}</span>
                                            <span class="text-6xl font-black text-white tracking-tighter uppercase italic">{{ $tier['price'] }}</span>
                                        </div>
                                        ...
                                        <a href="{{ !empty($tier['form_url']) ? $tier['form_url'] : ($event->builder_content['about']['registration_url'] ?? url('join')) }}" target="_blank" class="block w-full bg-brand-accent hover:bg-white text-slate-950 text-center py-6 rounded-sm font-black text-[10px] uppercase tracking-[0.3em] transition-all duration-700 italic group/btn-p">
                            ...
                            <div class="space-y-2 max-w-4xl mx-auto" x-data="{ openFaq: null }">
                                @foreach($event->builder_content['faq'] as $index => $q)
                                    <div class="bg-white border border-slate-100 rounded-sm overflow-hidden group/faq transition-all hover:bg-slate-950 shadow-sm">
                                        <button @click="openFaq = openFaq === {{ $index }} ? null : {{ $index }}" class="w-full flex items-center justify-between p-8 text-left">
                                            <span class="text-sm font-black text-slate-950 uppercase italic tracking-widest group-hover/faq:text-white transition-colors leading-tight">{{ $q['q'] }}</span>
                                            <i class="fa-solid fa-chevron-down text-slate-300 transition-all duration-700" :class="openFaq === {{ $index }} ? 'rotate-180 text-brand-accent' : ''"></i>
                                        </button>
                                        <div x-show="openFaq === {{ $index }}" x-collapse class="px-8 pb-8">
                        ...
                        <div id="resources" class="scroll-mt-40 animate-on-scroll">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                                @foreach($event->builder_content['resources'] as $res)
                                    <a href="{{ asset($res['url']) }}" target="_blank" class="group bg-slate-50 border border-slate-100 rounded-sm overflow-hidden hover:bg-slate-950 transition-all duration-700 flex flex-col relative shadow-sm">
                                        <div class="absolute top-0 left-0 w-0 h-1 bg-brand-accent group-hover:w-full transition-all duration-700 z-30"></div>
                                        <div class="relative aspect-video bg-slate-950 overflow-hidden border-b border-slate-100 group-hover:border-slate-800 transition-colors shadow-2xl">
                                            @if(isset($res['thumbnail']) && $res['thumbnail'])
                                                <img src="{{ asset($res['thumbnail']) }}" alt="{{ $res['title'] }}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000 group-hover:scale-105">
                                            @else
                        ...
                        <div id="venue" class="scroll-mt-40 bg-slate-950 border border-slate-900 rounded-sm overflow-hidden shadow-4xl animate-on-scroll group/venue relative">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 relative z-10">
                                <div class="p-12 md:p-20 flex flex-col justify-center">
                                    ...
                                    <h4 class="text-2xl font-black text-white mb-4 uppercase italic tracking-tighter leading-tight">{{ $event->builder_content['venue']['name'] }}</h4>
                                    <p class="text-slate-400 font-light italic text-xl leading-relaxed mb-12 border-l-2 border-white/10 pl-8">{{ $event->builder_content['venue']['address'] }}</p>

                                    <a href="https://maps.google.com/?q={{ urlencode($event->builder_content['venue']['address']) }}" target="_blank" class="inline-flex items-center gap-6 text-brand-accent font-black text-[10px] uppercase tracking-[0.4em] italic hover:text-white transition-all group/map">
                                ...
                                <div class="h-full bg-slate-900 relative min-h-[500px] border-l border-white/5">
                                     @if(isset($event->builder_content['venue']['image']) && $event->builder_content['venue']['image'])
                                        <img src="{{ asset($event->builder_content['venue']['image']) }}" class="w-full h-full object-cover grayscale group-hover/venue:grayscale-0 transition-all duration-[2000ms]">
                                     @else
                                ...
                                <div class="flex gap-2 items-center animate-logo-slide whitespace-nowrap">
                                    ...
                                    @foreach($slidingPartners as $partner)
                                        <div class="flex-none bg-slate-50 rounded-sm p-10 border border-slate-100 flex flex-col items-center justify-center h-48 w-64 group hover:bg-slate-950 transition-all duration-700 shadow-sm">
                                            <img src="{{ asset($partner['logo']) }}" alt="{{ $partner['name'] }}" class="max-h-16 max-w-full object-contain grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-700">
                                            @if(isset($partner['name']) && $partner['name'])
                                                <p class="mt-6 text-[8px] font-black text-slate-400 uppercase tracking-[0.3em] italic text-center group-hover:text-brand-accent transition-colors">{{ $partner['name'] }}</p>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    @if(isset($event->builder_content['cta']['form_url']) && $event->builder_content['cta']['form_url'])
                        <div id="cta" class="scroll-mt-40 bg-slate-950 border border-slate-900 rounded-sm p-16 md:p-32 text-center animate-on-scroll relative overflow-hidden group/final shadow-4xl">
                            ...
                            <div class="relative z-10 max-w-4xl mx-auto">
                                <h2 class="text-5xl md:text-8xl font-black text-white uppercase italic tracking-tighter mb-12 leading-[0.85]">{{ $event->builder_content['cta']['heading'] ?? 'Architect Your Excellence Node' }}</h2>
                                <p class="text-slate-400 font-light italic mb-16 text-2xl leading-relaxed max-w-3xl mx-auto border-l border-r border-white/5 px-12">{{ $event->builder_content['cta']['description'] ?? 'Join the global industrial transition. Submit your intelligence node and secure your summit pass today.' }}</p>

                                <a href="{{ $event->builder_content['cta']['form_url'] }}" class="inline-flex items-center justify-center w-full sm:w-auto gap-8 bg-brand-accent text-slate-950 hover:bg-white px-20 py-10 rounded-sm font-black text-[10px] uppercase tracking-[0.4em] italic shadow-4xl transition-all duration-700 group/final-btn">
