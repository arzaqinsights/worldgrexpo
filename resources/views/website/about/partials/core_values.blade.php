    ...
    <div class="container relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
            ...
            @foreach($power as $idx => $p)
                <div class="p-10 rounded-[3rem] bg-white border border-slate-100 shadow-xl group hover:bg-slate-950 transition-all duration-700 animate-on-scroll" style="transition-delay: {{ $idx * 100 }}ms">
                    <div class="space-y-8">
                        ...
                        <div class="space-y-4">
                            <div class="flex items-baseline gap-2">
                                <span class="text-4xl font-black text-brand-primary italic leading-none">{{ $p['l'] }}</span>
                                <h4 class="text-2xl font-black text-slate-900 group-hover:text-white uppercase tracking-tighter italic transition-colors leading-none">{{ $p['t'] }}</h4>
                            </div>
                            <p class="text-[10px] font-black text-slate-400 group-hover:text-slate-500 uppercase tracking-[0.2em] leading-relaxed transition-colors">{{ $p['d'] }}</p>
                        </div>
