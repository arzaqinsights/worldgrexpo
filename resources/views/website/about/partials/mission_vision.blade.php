      ...
      <div class="space-y-6 pt-8 border-t border-white/10">
       ...
       <div class="flex items-start gap-4">
        <div class="w-10 h-10 bg-slate-950 flex items-center justify-center shrink-0 border border-slate-950 shadow-sm">
         <i class="fa-solid fa-{{ $item['i'] }} text-brand-accent text-sm"></i>
        </div>
        <div class="space-y-1">
         <span class="block text-white font-black text-sm md:text-base uppercase tracking-tight">{{ $item['t'] }}</span>
         <span class="block text-slate-200 text-xs font-normal leading-relaxed">{{ $item['d'] }}</span>
        </div>
       </div>
       @endforeach
      </div>
