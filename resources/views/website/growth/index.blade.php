...
@section('content')
    ...
    <section class="relative pt-48 pb-24 bg-slate-950 text-white overflow-hidden">
        ...
        <div class="container relative z-10 text-left">
            <div class="inline-flex items-center gap-3 px-4 py-2 bg-slate-950 border-l-4 border-brand-accent mb-10">
                <span class="text-white text-[10px] font-black tracking-[0.4em] uppercase italic">Trajectory Metrics</span>
            </div>
            <h1 class="text-6xl md:text-8xl font-black uppercase mb-8 tracking-tighter leading-[0.85] italic">
                Conference <span class="text-brand-accent not-italic">Growth</span>
            </h1>
    ...
    <section class="py-32 bg-white relative overflow-hidden">
        <div class="container">
            <div class="text-left mb-24">
                <h2 class="text-5xl md:text-7xl font-black text-slate-950 uppercase italic leading-none tracking-tighter mb-8">
                    Conference <span class="text-brand-primary not-italic">Journey</span>
                </h2>
                ...
                @foreach($delegates as $index => $stat)
                    ...
                    <div class="flex-1 min-w-[150px] flex flex-col items-start group">
                        <div class="mb-6 text-left">
                            <span class="text-3xl font-black text-slate-200 group-hover:text-brand-accent transition-colors duration-700 italic">{{ $h['y'] ?? $stat->year }}</span>
                        </div>
                        <div class="w-full relative overflow-hidden shadow-2xl group-hover:bg-brand-primary transition-all duration-700 border border-slate-950 rounded-sm bg-slate-950"
                            ...
                            <div class="absolute inset-0 flex flex-col items-start justify-end p-8">
                                <div class="text-4xl font-black text-white leading-none tracking-tighter italic">{{ $stat->value }}</div>
                                <div class="text-[8px] font-black text-brand-accent uppercase tracking-[0.3em] mt-4 pt-4 border-t border-white/10 w-full">{{ $stat->label ?: 'Delegates' }}</div>
                            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                ...
                <div class="bg-slate-50 border border-slate-100 rounded-sm p-16 group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
                     ...
                     <h3 class="text-3xl font-black text-slate-950 uppercase italic mb-10 tracking-tighter group-hover:text-white transition-colors">
                        Speakers 
                        <span class="text-[10px] font-black text-slate-400 normal-case tracking-[0.3em] block mt-2">Institutional Halls</span>
                     </h3>
                     <div class="space-y-2">
                        ...
                        @foreach($speakers as $stat)
                            <div class="flex items-center justify-between p-8 border border-slate-200 {{ $stat->year == $latestSpeakerYear ? 'bg-slate-950 text-white border-slate-950' : 'bg-white' }} group-hover:bg-white/5 group-hover:border-white/10 group-hover:text-white transition-all rounded-sm">
                                <span class="font-black tracking-widest text-[10px] uppercase {{ $stat->year == $latestSpeakerYear ? 'text-brand-accent' : 'text-slate-400' }}">{{ $stat->year }}</span>
                                <span class="font-black text-4xl tracking-tighter italic">{{ $stat->value }}</span>
                            </div>
                        @endforeach
                     </div>
                </div>
                ...
                <div class="bg-slate-50 border border-slate-100 rounded-sm p-16 group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
                     ...
                     <h3 class="text-3xl font-black text-slate-950 uppercase italic mb-10 tracking-tighter group-hover:text-white transition-colors">
                        Excellence 
                        <span class="text-[10px] font-black text-slate-400 normal-case tracking-[0.3em] block mt-2">Institutional Sectors</span>
                     </h3>
                     <div class="space-y-2">
                        ...
                        @foreach($excellence as $stat)
                            <div class="flex items-center justify-between p-8 border border-slate-200 {{ $stat->year == $latestExcellenceYear ? 'bg-slate-950 text-white border-slate-950' : 'bg-white' }} group-hover:bg-white/5 group-hover:border-white/10 group-hover:text-white transition-all rounded-sm">
                                <span class="font-black tracking-widest text-[10px] uppercase {{ $stat->year == $latestExcellenceYear ? 'text-brand-accent' : 'text-slate-400' }}">{{ $stat->year }}</span>
                                <span class="font-black text-4xl tracking-tighter italic">{{ $stat->value }}</span>
                            </div>
                        @endforeach
                     </div>
        ...
        <div class="container relative z-10">
            <div class="text-left mb-24">
                <div class="inline-flex items-center gap-3 px-4 py-2 bg-white/5 border-l-4 border-brand-accent mb-10">
                    <span class="text-white text-[10px] font-black tracking-[0.4em] uppercase italic">Recognition & Honor</span>
                </div>
                <h2 class="text-5xl md:text-8xl font-black text-white uppercase italic leading-[0.85] tracking-tighter">
                   National <span class="text-brand-accent not-italic">Excellence</span> Awards
                </h2>
            </div>
                ...
                <div class="mb-2 px-0">
                    <div class="bg-white/5 border border-white/10 rounded-sm p-16 lg:p-24 relative overflow-hidden group hover:bg-white/10 transition-colors">
                        ...
                        <div class="text-left mb-20 relative">
                            <h4 class="text-4xl md:text-6xl font-black text-white mb-6 tracking-tighter italic uppercase leading-none">{{ $awardTitle }}</h4>
                            <p class="text-brand-accent text-[10px] font-black uppercase tracking-[0.5em] italic">Global Honor Node Statistics</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-2 relative">
                            @foreach($awardStats as $stat)
                                <div class="bg-white rounded-sm p-12 text-left shadow-2xl group/card hover:bg-brand-primary transition-all duration-700 min-h-[250px] border border-transparent">
                                    <div class="text-5xl md:text-7xl font-black text-slate-950 mb-6 tabular-nums tracking-tighter italic group-hover/card:text-white transition-colors">{{ $stat->value }}</div>
                                    <div class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] leading-relaxed group-hover/card:text-brand-accent transition-colors">{{ $stat->label ?: $stat->category }}</div>
                                </div>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="text-left mt-24 max-w-2xl">
                <p class="text-white/40 text-[10px] font-black uppercase tracking-[0.5em] italic leading-relaxed">
    ...
    @if(Auth::check() && Auth::user()->role === 'admin')
        <a href="{{ route('admin.growth.index') }}" class="fixed bottom-12 right-12 z-[100] bg-brand-accent text-slate-950 px-8 py-4 rounded-sm shadow-4xl hover:bg-white transition-all group flex items-center gap-4 border border-slate-950">
            ...
            <span class="font-black text-[10px] uppercase tracking-widest italic">Manage Metrics</span>
        </a>
    @endif
