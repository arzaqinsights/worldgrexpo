            ...
            @foreach($excellenceAwards as $award)
                <div class="group relative flex flex-col bg-white p-8 transition-all duration-500 hover:bg-slate-50">
                    ...
                    <div class="relative aspect-[4/5] overflow-hidden mb-10 bg-slate-100">
                        <img src="{{ asset($award->award_image) }}" alt="{{ $award->title }}" class="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105">
                        ...
                        <div class="absolute bottom-0 left-0 bg-brand-primary p-6 w-full translate-y-full group-hover:translate-y-0 transition-transform">
                            <p class="text-white font-black text-sm leading-tight tracking-widest uppercase">{{ $award->title }}</p>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center gap-5">
                            @if($award->giver_image)
                                <div class="w-14 h-14 bg-slate-50 overflow-hidden shrink-0 border border-slate-100">
                                    <img src="{{ asset($award->giver_image) }}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all">
                                </div>
                            @endif
                            <div>
                                <p class="text-xs font-black text-slate-900 leading-tight mb-1 uppercase tracking-wider">{{ $award->giver_name }}</p>
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.15em] leading-relaxed">{{ Str::limit($award->giver_designation, 40) }}</p>
                            </div>
                        </div>
