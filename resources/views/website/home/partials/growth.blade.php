...
<section class="py-32 bg-white relative overflow-hidden">
    ...
    <div class="container relative z-10">
                        ...
                        @foreach($delegates as $index => $stat)
                            ...
                            <div class="flex-1 flex flex-col items-center group/bar cursor-pointer">
                                ...
                                <div class="mb-6 transition-transform group-hover/bar:-translate-y-2">
                                    <span class="text-xs md:text-sm font-black {{ $isLatest ? 'text-brand-accent' : 'text-slate-400' }} tracking-widest uppercase">{{ $stat->year }}</span>
                                </div>

                                <!-- The Bar -->
                                <div class="w-full relative overflow-hidden transition-all duration-700 {{ $isLatest ? 'shadow-[0_20px_50px_rgba(170,204,0,0.3)]' : 'shadow-xl' }} group-hover/bar:-translate-y-4 rounded-sm"
                                     style="height: {{ ($h / 100) * 350 }}px; background: {{ $isLatest ? 'var(--color-brand-accent)' : 'var(--color-brand-primary)' }};">
                                    ...
                                    <div class="absolute inset-0 flex flex-col items-center justify-center text-center px-1">
                                        <div class="text-xl md:text-3xl font-black {{ $isLatest ? 'text-brand-primary-dark' : 'text-white' }} leading-none tracking-tighter">{{ $stat->value }}</div>
                                        <div class="text-[8px] font-black {{ $isLatest ? 'text-brand-primary-dark/60' : 'text-white/40' }} uppercase tracking-widest mt-3 border-t {{ $isLatest ? 'border-brand-primary-dark/10' : 'border-white/10' }} pt-3 hidden md:block">
                                            {{ $stat->label ?: 'Visitors' }}
                                        </div>
                                    </div>

                                    <!-- Bottom Light Gradient -->
                                    <div class="absolute bottom-0 left-0 w-full h-1.5 bg-white/30 {{ $isLatest ? '' : 'hidden' }}"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="mt-24 flex flex-col md:flex-row items-center justify-center gap-12 animate-on-scroll delay-300">
             <a href="{{ route('growth') }}" class="group relative px-12 py-6 bg-slate-950 text-white rounded-sm overflow-hidden shadow-2xl transition-all hover:-translate-y-1">
             ...
             <div class="text-center md:text-left">
                <p class="text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] mb-2 italic">
                    ( Including {{ count(config('sectors.sectors')) }}+ Global Sectors )
                </p>
