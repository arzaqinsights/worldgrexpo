...
<?php $__env->startSection('title', 'User Intelligence'); ?>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-sm text-slate-700 divide-y divide-slate-100">
                <?php $__empty_1 = true; $__currentLoopData = $users->where('role', 'user'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-slate-50/80 transition-all group">
                        <td class="p-6 pl-8">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-sm <?php echo e($user->role === 'admin' ? 'bg-purple-950 text-purple-400' : 'bg-brand-primary text-white'); ?> flex items-center justify-center font-black relative border border-white/10 shadow-lg">
                                ...
                                <div>
                                    <h3 class="font-black text-slate-950 uppercase tracking-tighter"><?php echo e($user->name); ?></h3>
                                    <p class="text-[9px] uppercase font-black tracking-widest text-brand-primary mt-1"><?php echo e($user->role); ?> NODE</p>
                                </div>
                            </div>
                        </td>
                        <td class="p-6">
                            <span class="font-black text-slate-950 uppercase tracking-tighter text-base block mb-1"><?php echo e($user->company_name ?: 'INDEPENDENT'); ?></span>
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest"><?php echo e($user->designation ?: 'Professional'); ?></span>
                        </td>
                        <td class="p-6">
                            <div class="flex flex-col gap-2">
                                <span class="font-bold text-slate-600 text-xs flex items-center gap-2"><i class="fa-regular fa-envelope text-brand-primary"></i> <?php echo e($user->email); ?></span>
                                <span class="text-[10px] font-black text-slate-400 flex items-center gap-2"><i class="fa-solid fa-phone text-brand-primary"></i> <?php echo e($user->phone_number ?: 'UNAVAILABLE'); ?></span>
                            </div>
                        </td>
                        <td class="p-6">
                            <?php if($user->submissions_count > 0): ?>
                                <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-sm bg-brand-primary/10 border border-brand-primary/20 text-brand-primary text-[10px] font-black uppercase tracking-widest shadow-sm">
                                    <i class="fa-solid fa-file-waveform"></i> <?php echo e($user->submissions_count); ?> Active
                                </div>
                            <?php else: ?>
                        ...
                        <td class="p-6 pr-8 text-right">
                            <a href="<?php echo e(route('admin.users.show', $user->id)); ?>" class="inline-flex items-center justify-center w-10 h-10 rounded-sm bg-slate-950 text-white hover:bg-brand-accent hover:text-brand-primary-dark transition-all shadow-xl group-hover:scale-110" title="Inspect Node">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\worldgrexpo\resources\views/admin/users/index.blade.php ENDPATH**/ ?>