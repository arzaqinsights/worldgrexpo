        ...
        <div class="container space-y-32">
                ...
                <div class="w-full lg:w-1/2 space-y-12 animate-on-scroll">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        ...
                        <div class="flex items-center gap-6 p-6 bg-slate-50 border border-slate-100 rounded-sm group hover:bg-slate-950 transition-all duration-500">
                            ...
                            <span class="text-[10px] font-black uppercase text-slate-950 tracking-widest group-hover:text-white transition-colors"><?php echo e($item); ?></span>
                        </div>
            ...
            <div class="grid md:grid-cols-3 gap-2">
                ...
                <div class="group relative bg-white p-16 rounded-sm border border-slate-100 hover:bg-slate-950 transition-all duration-700">
                    <div class="w-20 h-20 bg-slate-50 border border-slate-200 rounded-sm flex items-center justify-center text-slate-950 mb-10 group-hover:bg-brand-accent group-hover:border-brand-accent transition-all duration-700">
                        <i class="fa-solid fa-<?php echo e($step['icon']); ?> text-3xl group-hover:scale-110"></i>
                    </div>
                    <h4 class="text-3xl font-black text-slate-950 mb-6 uppercase italic tracking-tighter group-hover:text-white transition-colors"><?php echo e($step['t']); ?></h4>
                    <p class="text-slate-500 text-xs font-bold leading-relaxed uppercase tracking-widest group-hover:text-slate-400 transition-colors"><?php echo e($step['d']); ?></p>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
                ...
                <div class="relative z-10 flex flex-col lg:flex-row items-center gap-24">
                    ...
                    <div class="lg:w-1/2 grid grid-cols-2 gap-4">
                        ...
                        <div class="p-10 bg-white/5 border border-white/10 rounded-sm hover:bg-brand-primary transition-all duration-700 group">
                            <i class="fa-solid fa-<?php echo e($dev['icon']); ?> text-3xl mb-8 text-brand-accent group-hover:text-slate-950 transition-colors"></i>
            ...
            <div class="flex flex-col lg:flex-row items-center gap-24">
                <div class="w-full lg:w-1/2 order-2 lg:order-1 grid gap-2">
                    ...
                    <div class="bg-slate-50 p-10 rounded-sm border border-slate-100 flex gap-10 items-center group hover:bg-slate-950 transition-all duration-500">
                        <span class="text-6xl font-black text-slate-200 italic group-hover:text-brand-accent transition-colors leading-none"><?php echo e($struct['n']); ?></span>
                        <div>
                            <h5 class="font-black text-slate-950 uppercase text-xl mb-2 tracking-tighter group-hover:text-white transition-colors italic"><?php echo e($struct['t']); ?></h5>
                            <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest group-hover:text-slate-400 transition-colors leading-relaxed"><?php echo e($struct['d']); ?></p>
                        </div>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
                <div class="w-full lg:w-1/2 order-1 lg:order-2 space-y-12">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <?php $__currentLoopData = ['Pre-screening', 'Lender Node Connect', 'Transition Camps', 'Mentorship']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <span class="px-6 py-3 bg-slate-950 text-white rounded-sm text-[10px] font-black uppercase tracking-[0.3em] italic border border-white/10"><?php echo e($tag); ?></span>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                </div>
            </div>
            ...
            <div class="text-center py-32 border-t border-slate-100">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                    <a href="<?php echo e(route('contact')); ?>" class="w-full sm:w-auto px-16 py-8 bg-slate-950 text-white font-black uppercase tracking-[0.3em] text-xs rounded-sm hover:bg-brand-accent hover:text-slate-950 transition-all shadow-4xl italic group">
                        Apply for Funding Node <i class="fa-solid fa-arrow-right group-hover:translate-x-4 transition-transform ml-4"></i>
                    </a>
                    <a href="<?php echo e(route('register')); ?>" class="w-full sm:w-auto px-16 py-8 bg-white border border-slate-200 text-slate-950 font-black uppercase tracking-[0.3em] text-xs rounded-sm hover:bg-slate-50 transition-all italic">
                        Acquire Membership
                    </a>
                </div>
            </div>
