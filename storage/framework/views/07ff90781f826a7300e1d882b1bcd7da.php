   ...
   <div class="bg-slate-950 p-8 md:p-12 flex flex-col justify-between space-y-8">
    <div class="space-y-6">
     ...
     <div class="grid grid-cols-1 gap-6">
      ...
      <div>
       <span class="block text-2xl md:text-3xl font-black text-white tracking-tight"><?php echo e(count(config('sectors.sectors'))); ?></span>
   ...
   <?php $__currentLoopData = $focus; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <div class="bg-white p-6 md:p-8 group hover:bg-slate-50 transition-all">
     <div class="w-10 h-10 bg-slate-50 flex items-center justify-center mb-6 group-hover:bg-brand-primary group-hover:text-white transition-all border border-slate-100">
      <i class="fa-solid <?php echo e($item['icon']); ?> text-sm text-slate-700 group-hover:text-white"></i>
     </div>
     <h4 class="text-xs font-black uppercase tracking-wider text-slate-900 mb-1"><?php echo e($item['label']); ?></h4>
     <p class="text-xs text-slate-400 font-medium"><?php echo e($item['desc']); ?></p>
    </div>
