...
<?php $__env->startSection('content'); ?>
 ...
 <section class="relative pt-40 pb-20 bg-slate-950 overflow-hidden border-b border-slate-900">
  ...
  <div class="absolute inset-0 opacity-20">
   <img src="<?php echo e(asset('images/sectors/chemicals-petrochemicals-industry.jpg')); ?>" class="w-full h-full object-cover">
  </div>
  ...
  <div class="container space-y-20">
     ...
     <?php $__currentLoopData = $segments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <div class="p-8 bg-slate-50 rounded-2xl text-center group hover:shadow-lg transition-all relative overflow-hidden">
       ...
       <div class="w-20 h-20 mx-auto mb-10 bg-slate-50 border border-slate-100 flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm duration-500">
        <i class="fa-solid fa-<?php echo e($s['icon']); ?> text-3xl font-bold"></i>
       </div>
       <p class="text-[10px] font-bold text-slate-900 group-hover:text-white uppercase tracking-[0.3em] leading-tight transition-colors"><?php echo e($s['label']); ?></p>
      </div>
      ...
      <div class="space-y-8 pt-12 border-t border-white/5">
       <?php $__currentLoopData = [
        ...
        <div class="flex items-center gap-8 group/item">
         ...
         <span class="text-[12px] font-bold uppercase text-slate-400 tracking-[0.3em] group-hover/item:text-white transition-colors duration-500 leading-tight"><?php echo e($stat); ?></span>
        </div>
       <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </div>
     </div>
   ...
   <div class="grid lg:grid-cols-2 gap-32 items-center">
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 shadow-sm">
     ...
     <div class="p-8 bg-slate-50 rounded-2xl text-center group hover:shadow-lg transition-all relative overflow-hidden">
      <div class="absolute top-0 left-0 w-1 h-full bg-indigo-600 scale-y-0 group-hover:scale-y-100 transition-transform origin-top duration-500"></div>
      <p class="text-[10px] font-bold uppercase text-indigo-600 group-hover:text-indigo-400 tracking-[0.4em] mb-6 transition-colors">Segment 0<?php echo e($idx + 1); ?></p>
      <p class="text-3xl font-bold text-slate-950 group-hover:text-white leading-tight mb-4 transition-colors"><?php echo e($pill['t']); ?></p>
      <p class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] transition-colors"><?php echo e($pill['d']); ?></p>
     </div>
     <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
    ...
    <div class="space-y-16">
     ...
     <div class="space-y-10 pt-8">
      ...
      <div class="flex items-center gap-10 group">
       ...
       <p class="text-[14px] font-bold uppercase text-slate-700 tracking-[0.2em] transition-all duration-500 group-hover:translate-x-6 leading-tight group-hover:text-slate-950"><?php echo e($item); ?></p>
      </div>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
     </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 shadow-sm">
     <?php $__currentLoopData = ['Packaging & FMCG', 'Agriculture', 'Automotive', 'Construction', 'Healthcare', 'Textiles']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $eco): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <div class="p-12 bg-white text-center group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
        <div class="absolute top-0 left-0 w-full h-1 bg-indigo-600 scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-500"></div>
        <div class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-600 group-hover:text-white transition-colors leading-tight"><?php echo e($eco); ?></div>
      </div>
     <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
   </div>
    ...
    <div class="space-y-16">
     ...
     <div class="space-y-px bg-slate-200 border border-slate-200 shadow-sm">
      ...
      <div class="flex items-center gap-10 p-12 bg-white group hover:bg-slate-950 transition-all duration-700 relative overflow-hidden">
       ...
       <span class="text-[14px] font-bold uppercase tracking-[0.15em] text-slate-900 group-hover:text-white transition-colors leading-tight"><?php echo e($imp); ?></span>
      </div>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
     </div>
    </div>
    ...
    <div class="p-8 lg:p-12 bg-slate-950 text-white space-y-16 relative overflow-hidden group shadow-sm">
     ...
     <div class="relative z-10 space-y-12">
      ...
      <div class="grid grid-cols-2 gap-px bg-white/5 pt-12">
        ...
        <span class="p-6 bg-slate-950 border border-white/5 text-[10px] font-bold uppercase tracking-[0.4em] text-indigo-300 text-center hover:bg-indigo-600 hover:text-slate-950 transition-all cursor-default"><?php echo e($tag); ?></span>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </div>
     </div>
    </div>
