     ...
     <article class="group relative flex flex-col rounded-2xl overflow-hidden bg-slate-50 border border-slate-100 shadow-xl transition-all duration-700 hover:-translate-y-4 hover:shadow-brand-primary/20">
       ...
       <?php if($article->category): ?>
        <div class="absolute top-6 left-6 flex flex-wrap gap-2 z-20">
         <?php $__currentLoopData = array_map('trim', explode(',', $article->category)); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <span class="px-4 py-1.5 bg-brand-primary text-white text-[8px] font-bold uppercase tracking-widest rounded-full shadow-lg"><?php echo e($cat); ?></span>
      ...
      <div class="p-10 flex flex-col flex-1 space-y-6">
       ...
       <div class="mt-auto pt-6 border-t border-slate-100 flex items-center justify-between">
        <a href="<?php echo e(route('blog.show', $article->slug)); ?>" class="text-[10px] font-bold text-slate-900 group-hover:text-brand-primary transition-colors flex items-center gap-4 uppercase tracking-[0.3em]">
